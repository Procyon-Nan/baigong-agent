...
import { getAdminConversationExecutionIndex } from "@/src/server/conversations/service";
import { parseConversationId } from "@/src/server/http/p3-conversation-schemas";
import { parseAdminConversationExecutionQuery } from "@/src/server/http/p4-admin-conversation-schemas";
...
export async function GET(
  ...
  return handleRoute(async () => {
    ...
    const query = parseAdminConversationExecutionQuery(
      new URL(request.url).searchParams,
    );
    return jsonResponse(
      await getAdminConversationExecutionIndex(
        principal,
        conversationId,
        query,
      ),
